...
setwd(r"(C:/Users/Valen/Documents/Git-Repositorys/TUHH/Applied Statistics/week01)")
...
```{r}
# trim str
survey <- survey |> mutate(Drinks.Coffee = str_trim(Drinks.Coffee))
...
```{r}
png("Presentation/DrinksCoffee.png", width = 800, height = 400)

barplot(table(survey$Drinks.Coffee),
        main = "Do You Buy Coffee on Campus?",
        ylab = "No. of Students",
        col = c("red", "green"))
dev.off()
...
```{r}
mentions_plot <- ggplot(location_stats, aes(x = N, y = mean_rating)) +
  geom_point(size = 3, color = "#2196F3") +
  geom_smooth(method = "lm", color = "#E91E63", linewidth = 1) +
  labs(title = "How does the Popularity of a Location influence its rating?",
       subtitle = "Mean location rating in relation to the amount of mentions N", 
       x = "Number of mentions (N)",
       y = "Mean Rating") +
  theme_minimal()


ggsave("Presentation/mentions_rating_relation.png",
       width = 8, height = 5, dpi = 300)
```
